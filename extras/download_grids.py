...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Install pre-computed Synthesizer grids.')

    parser.add_argument("-grid", type=str, help = 'grid code (use -list_grids to find available grids)', default = None)
    parser.add_argument("-dir", "--directory", type=str, default = default_grid_dir, help='grid directory location, defaults to synthesizer install directory')
    ...
    if args.list_grids:
        ...
        if args.grid:
            gdown.download(grid_url[args.grid], output=f'{args.directory}/{args.grid}.h5', quiet=False, fuzzy=True)
        else:
